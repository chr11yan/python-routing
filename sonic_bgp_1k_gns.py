#!/usr/bin/env python3
"""
sonic_bgp_1000_routes.py  (v3 — all fixes integrated)
════════════════════════════════════════════════════════════════════
Generates 1000 BGP routes between two Dell SONiC switches.
...
# ─────────────────────────────────────────────────────────────────
VTYSH_EXEC_PROMPT   = r"\w[\w-]*#\s"
VTYSH_CONFIG_PROMPT = r"\w[\w-]*\(config\)#\s"
VTYSH_ROUTER_PROMPT = r"\w[\w-]*\(config-router\)#\s"
VTYSH_AF_PROMPT     = r"\w[\w-]*\(config-router-af\)#\s"
VTYSH_IF_PROMPT     = r"\w[\w-]*\(config-if\)#\s"       # FIX G: interface mode
VTYSH_ANY_PROMPT    = r"\w[\w-]*(?:\([^)]*\))?#\s"
SHELL_PROMPT        = r"\w[^@]*@\S+[#$]\s"
...
def run_vtysh_config(conn, config_block, label=""):
    ...
    def _read_until(pattern, timeout=20):
        """Read until pattern — \s not $ so matches immediately."""
        try:
            ...
            log.debug(
                "  [%s] read_until timeout pattern='%s': %s",
    ...
    try:
        ...
        time.sleep(1.5)
        init = _read_until(VTYSH_EXEC_PROMPT, timeout=15)
...
def connect_switch(device, label,
    ...
    conn_params = {
        ...
        "session_timeout": device.get("session_timeout", 120),
        "banner_timeout":  30,
    }
...
def configure_bgp_switch2(conn, device, label):
    ...
    config_block = [
        ...
        f"neighbor {device['bgp_peer_ip']} timers 10 30",
        f"neighbor {device['bgp_peer_ip']} timers connect 10",
        f"neighbor {device['bgp_peer_ip']} maximum-prefix 5000 80 warning-only",
        "address-family ipv4 unicast",
        f"neighbor {device['bgp_peer_ip']} activate",
        "exit-address-family",
        "exit",   # router -> config
...
# ─────────────────────────────────────────────────────────────────
def wait_for_bgp_convergence(conn2, expected_routes=1000, max_wait=300):
    ...
    log.warning("  [WARN] Convergence timeout")
    return False
...
def verify_switch2(conn, device):
    ...
    out = run_vtysh(conn, "show bgp ipv4 unicast 10.3.231.0/24", label)
    log.info("\n%s", out)

    log.info("\n[SW2] Middle route ~500th (10.1.244.0/24):")
    out = run_vtysh(conn, "show bgp ipv4 unicast 10.1.244.0/24", label)
    log.info("\n%s", out)
...
|  vtysh -c "show bgp ipv4 unicast summary"                  |
|  vtysh -c "show bgp ipv4 unicast" | grep "^*>" | wc -l    |
|  vtysh -c "show bgp ipv4 unicast 10.3.231.0/24"           |
|  vtysh -c "show bgp ipv4 unicast neighbors                 |
